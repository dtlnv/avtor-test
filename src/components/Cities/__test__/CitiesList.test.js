...
describe('Cities list test', () => {
    ...
    it('Renders without list', () => {
        ...
        expect(container.querySelectorAll('.city-in-search').length).toBe(0);
    });

    it('Renders with list', () => {
        const { container } = render(<TestAppWrapper><CitiesList list={dummyList} /></TestAppWrapper>);

        expect(container.querySelectorAll('.city-in-search').length).toBe(dummyList.length);
    });
    ...
    it('Renders correct list', () => {
        ...
        for (let i = 0; i < dummyList.length; i++) {
            expect(container.querySelectorAll('.city-in-search .city-name')[i].textContent).toBe(dummyList[i].formatted);
        }
    });
